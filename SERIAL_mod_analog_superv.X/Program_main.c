...
void conv_AN0()
{
    ...
    VdigADC = (VdigADC << 8) + ADRESL;      // Mantém os 2 bits + significativos e soma os 8 bits - significativos do ADRES
...
void __interrupt() receive_DATA(void) //Interrupção para leitura
{
    if(PIR1bits.RCIF == 1)
    { 
        ...
        {
            if(BufferSERIALRX[0] == '1') LED_A = 1;    
            else if(BufferSERIALRX[0] == '0')LED_A = 0;       
            
            if(BufferSERIALRX[1] == '1')LED_V = 1;    
            else if(BufferSERIALRX[1] == '0')LED_V = 0;       
